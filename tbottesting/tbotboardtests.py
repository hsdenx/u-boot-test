...
def _tbottest_one_board(
    ...
    try:
        ...
        tbot.log.message(tbot.log.c(f"buildman build for {b['boardname']} failed with exception " + str(e)).red)
        return 1

    try:
        bmcfg.bm_copy_results2lab()
    except Exception as e:
        tbot.log.message(tbot.log.c(f"copy images for {b['boardname']} failed with exception " + str(e)).red)
        return 1
